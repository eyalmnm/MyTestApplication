...
/**
 * Created by eyalmuchtar on 6/11/16.
 */


// Ref: http://stackoverflow.com/questions/4381033/multi-gradient-shapes
// Ref: http://stackoverflow.com/questions/14294532/canvas-drawtext-direction

public class ColumnsGaugeViewBase extends View {
    ...
    private int maxColumnWidth = 20;   // TODO Read from View's properties
    private int numberOfColumnsInGroup = 3; // TODO Read from View's properties
    ...
    public ColumnsGaugeViewBase(Context context) {
        super(context);

        init();
    }

    public ColumnsGaugeViewBase(Context context, AttributeSet attrs) {
        super(context, attrs);

        init();
    }

    public ColumnsGaugeViewBase(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setData(ArrayList<Float> data, ArrayList<String> titles) throws Exception {
        if (true == checkDataValidity(data, (ArrayList) titles)) {
            this.data = data;
            this.titles = titles;
        }
    }

    private boolean checkDataValidity(ArrayList<Float> data, ArrayList titles) throws Exception {
    ...
    private float calculateMinValueY(float graphHeight) {
        return ((graphHeight * 100) / 120) * (10F / 100);   // Leave buttom space which is 10% of the value
    }
